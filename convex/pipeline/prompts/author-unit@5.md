...
- Usually write **3-6 cards per unit**. Use only 2 cards when the unit is
  genuinely short (typically <=2 narration sentences) and additional cards
  would be repetitive.
- Each card is anchored to narration with `enterAt`:
  `narration` = sentence id, `word` = a word that appears verbatim in that
  sentence (the card enters when that word is spoken).
...
- **Title -> content handoff:** keep the title-card on sentence 1, then make
  the first content card anchor to sentence 2 (`n2`) using the first spoken
  word of `n2` for a clean boundary transition.
- **Maintain card cadence.** Prefer a fresh visual beat every 2-5 seconds of
  speech. Avoid long dead air where one card stays up while multiple
  narration sentences pass.
- Choose the most specific template that fits; use `text-card` only as a
  ...
  speaks.
- When the course presentation is avatar-enabled, set `visualTreatment` to
  `"avatar-overlay"` only for templates identified as overlay-capable in the
  manifest below. The title-card must be avatar-overlay. Use the treatment
  sparingly for content cards: preserve a mixed sequence of ordinary opaque
  cards and avatar overlays, never force every card over the presenter.
...
| template | use for | avatar overlay | typical props |
|----------|---------|---------------|
| title-card | module/unit opener | always in avatar mode | kicker, title, courseLabel |
| stat-card | one headline number | yes | headline, supporting, sourceLabel, bgAssetRef?, bgTreatment? |
| list-reveal | 2-5 items revealed in sequence | yes | heading, items[{text, sourceLabel?}], bgAssetRef?, bgTreatment? |
